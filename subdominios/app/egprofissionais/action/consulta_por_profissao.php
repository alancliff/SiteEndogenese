<?php
/* ========================================================================
 * EG Profissionais - consulta_por_profissao.php  V. 1.0
 * Endogênese Consultoria e Serviços LTDA
 * ========================================================================
 * Finalidade: Cria a lista de profissionais para edição, conforme a profissão escolhida.
 * Autor: Alan Cliff
...
if ($profissao == "todas") {
	$sql_geral = $conexao->prepare("SELECT id_profissional, nome_conhecido, profissao
				FROM profissionais
				WHERE ativa = true
				");
}
...
if ($profissao == "todas") {
	$sql_geral = ("SELECT id_profissional, nome_conhecido, profissao
				FROM profissionais
				WHERE ativa = true
...
$sql_geral->execute();
	...
	if ($sql_geral->rowCount() > 0)
		{	
			...
			echo "<div class='row'>";
			while ($profissional = $sql_geral->fetch(PDO::FETCH_ASSOC) )
					{
			...
			if ($num_pagina >= 2) 
			{
				...
				{
					echo "<li>
							<a onclick='ConsultaProfissao($pagina_atual - 1, \"$profissao\")' aria-label='Previous'>
				        		<span aria-hidden='true'>&laquo;</span>
				      		</a>
				    	  </li>";
				}
				...
				if ($num_pagina <= $max_pagina) 
				{
					for ($i=1; $i <= $num_pagina ; $i++)
					{
						...
						{
							echo " <li ><a onclick='ConsultaProfissao($i, \"$profissao\")'>$i</a></li>";
						}
				...
				else
				{
					for ($i=1; $i <= $max_pagina ; $i++)
					{
						...
						elseif ($ultima_pg > $num_pagina)
						{
								...
								{
									echo " <li ><a onclick='ConsultaProfissao($mysql_a, \"$profissao\")'>$mysql_a</a></li>";
								}
						}
						...
						else
						{
								...
								{
									echo " <li ><a onclick='ConsultaProfissao($mysql_b, \"$profissao\")'>$mysql_b</a></li>";
								}
						}
				...
				{
					echo "<li>
				      		<a onclick='ConsultaProfissao($pagina_atual + 1, \"$profissao\")' aria-label='Next'>
				        		<span aria-hidden='true'>&raquo;</span>
				      		</a>
				      	  </li>";
				}
