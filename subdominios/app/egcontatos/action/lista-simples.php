...
if ($consultor == "" && $prioridade !=="") {
	$sql =  ("SELECT a.nome_contato, b.* 
		FROM contato a INNER JOIN prospeccao b
		ON a.id_contato = b.id_contato
		WHERE b.prioridade = '$prioridade'");	
}
...
if ($consultor !== "" && $prioridade =="") {
	$sql =  ("SELECT a.nome_contato, b.* 
		FROM contato a INNER JOIN prospeccao b
		ON a.id_contato = b.id_contato
		WHERE b.consultor_responsavel = '$consultor'");	
}
...
$sql_geral = $sql." LIMIT $registros_por_pagina OFFSET $inicio";
// echo $sql_geral;
	...
	{	
		...
		$class_item = "bg-info";
			...
			if ($num_pagina >= 2) 
			{
				...
				{
					echo "<li>
							<a onclick='listaSimples($pagina_atual - 1)' aria-label='Previous'>
				        		<span aria-hidden='true'>&laquo;</span>
				      		</a>
				    	  </li>";
				}
				...
				if ($num_pagina <= $max_pagina) 
				{
					for ($i=1; $i <= $num_pagina ; $i++)
					{
						...
						{
							echo " <li ><a onclick='listaSimples($i)'>$i</a></li>";
						}
				...
				else
				{
					for ($i=1; $i <= $max_pagina ; $i++)
					{
						...
						elseif ($ultima_pg > $num_pagina)
						{
								...
								{
									echo " <li ><a onclick='listaSimples($mysql_a)'>$mysql_a</a></li>";
								}
						}
						...
						else
						{
								...
								{
									echo " <li ><a onclick='listaSimples($mysql_b)'>$mysql_b</a></li>";
								}
						}
				...
				{
					echo "<li>
				      		<a onclick='listaSimples($pagina_atual + 1)' aria-label='Next'>
				        		<span aria-hidden='true'>&raquo;</span>
				      		</a>
				      	  </li>";
				}
